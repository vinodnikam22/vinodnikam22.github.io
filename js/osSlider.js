    ...
    that.gridTop = function(tid,showNum) {
        ...
        for (var i = 0; i < 12; i++) {//利用循环 创建出栅格节点
            ...
            $cvNode.css({//为每个栅格节点添加css样式
                ...
                'zIndex':20,
                'overflow':'hidden',
                'left':that.width/1*i+'px',
                'top':'0'
            });
            $cvNode.find('*').first().css({
                'display':'block',
                'margin-left':that.width/-1*i+'px'
            });
        }
    ...
    that.gridLeft = function(tid,showNum) {
        ...
        for (var i = 0;i<12;i++) {//利用循环 创建出栅格节点
            ...
            $cvNode.css({//为每个栅格节点添加css样式
                ...
                'overflow':'hidden',
                'left':'0',
                'top':that.height/1*i+'px',
            });
            $cvNode.find('*').first().css({
                'display':'block',
                'margin-top':that.height/-1*i+'px'
            });
        }
        switch (showNum) {
            default:
    ...
    that.cellToggle = function(tid) {
        ...
        for (var i = 0;i<20;i++) {//利用循环 创建出栅格节点
            ...
            $cvNode.css({//为每个栅格节点添加css样式
                ...
                'zIndex':20,
                'overflow':'hidden',
                'left':that.width/1*(i%5)+'px',
                'top':that.height/1*rows+'px',
            });
            $cvNode.find('*').first().css({
                'display':'block',
                'margin-left':that.width/-1*(i%5)+'px',
                'margin-top':that.height/-1*rows+'px',
            });
        }
        //添加动画过渡效果
        $(that.cNodes[that.nowNodeKey]).find('.cvNode').each(function(index,el){
